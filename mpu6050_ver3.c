...
***********************************************/
int processRotation(unsigned char *buffer) {
  float x,y,z,tmp;
  short int i;
  struct timeval t;
  gettimeofday(&t,NULL);
...
***********************************************/
void processIMUchar(unsigned char c, int number) {
  ...
  *********************/
  ...
  switch(buffer[1]) {
    case 0x51:
        ...
        return;
    case 0x52:
		if (number == 1) tmp = processRotation(buffer);
		else tmp2 = processRotation(buffer);
		ctmp = tmp - tmp2;
		ctmp = sqrt(ctmp*ctmp);
		if (ctmp > 50) {
			for (int qwe = 0;qwe < 20;qwe++) {
				printf("Warning!\n");
			}
			LEDon();
		}
